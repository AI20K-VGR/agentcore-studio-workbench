...
| `tool-call` | Tuỳ tool tích hợp — hiện là stub, chưa gọi API ngoài thật nào | **31µs trung bình** (stub, gần free) — 0 token vì chưa có tool thật nào tốn LLM call | Cần hành động ngoài hệ thống (hiện chỉ whitelist `kb_search`, xem `validator.py` luật 7) — **honest gap**: số đo này KHÔNG đại diện cho tool thật (vd gọi API bên thứ 3 sẽ có độ trễ mạng riêng, chưa đo được vì chưa có tool nào khác `kb_search`) |
