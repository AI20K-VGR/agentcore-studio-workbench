...
`builder.py` (which only ever constructs a fresh one from scratch). Single canonical home for
"inject/strip the per-turn `query` on every `kb-retrieve` node" — before this module existed,
`apps/studio` carried TWO independent, hand-written copies of this exact operation
(`routes/chat.py::_with_query`, `eval_adapter.py::_with_query`/`_map_kb_params`) that had quietly
DRIFTED apart: chat's patched EVERY `kb-retrieve` node in the DAG, eval_adapter's patched only the
FIRST one. A recipe with 2+ `kb-retrieve` nodes (nothing in `validator.graph_lint`'s 7 rules
forbids that — canvas lets an admin wire up as many as they want) would therefore behave
differently depending on whether it was reached via chat or via eval/publish: exactly the kind of
"the recipe that got SCORED isn't the recipe that RUNS" gap `DEC-D20-07` closed on the harness side
of the pipeline, reopened on the chat side by two copies of the same idea disagreeing.
...
it by coincidence, because `create_recipe_d4`'s fixed 6-node DAG happens to have exactly one
`kb-retrieve` node)."""
...
def _map_kb_retrieve_params(recipe: Recipe, fn: Callable[[dict[str, object]], dict[str, object]]) -> Recipe:
    ...
    unchanged — this never raises for that shape."""
    new_nodes = [
        node.model_copy(update={"params": fn(dict(node.params))}) if node.type is NodeType.KB_RETRIEVE else node
