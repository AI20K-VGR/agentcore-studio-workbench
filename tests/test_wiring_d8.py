...
Validates:
1. resolve_tenant_id() — happy path: reads tenant_id server-side from session.
2. resolve_tenant_id() — fail-closed: missing tenant_id raises PermissionError.
3. resolve_tenant_id() — fail-closed: None tenant_id raises PermissionError.
4. resolve_tenant_id() — fail-closed: blank tenant_id raises PermissionError.
5. resolve_tenant_id() — T1 IDOR prevention: client payload tenant ignored.
6. resolve_tenant_id() — alias key "tenant" is accepted as fallback.
7. resolve_tenant_id() — non-mapping session raises TypeError (not PermissionError).
...
# ===========================================================================
# resolve_tenant_id() — Group 1: Happy path
# ===========================================================================


def test_resolve_tenant_id_returns_tenant_id_from_session() -> None:
    """DoD 1: resolve_tenant_id() reads tenant_id server-side from the session dict.
    ...
    result = resolve_tenant_id(session)
    assert result == ANKOR_ID
    assert isinstance(result, UUID)


def test_resolve_tenant_id_accepts_tenant_alias_key() -> None:
    """DoD 6: resolve_tenant_id() supports 'tenant' as a fallback alias for 'tenant_id'."""
...
def test_resolve_tenant_id_accepts_uuid_string() -> None:
    """resolve_tenant_id() parses a UUID-formatted string into a UUID object."""
...
# ===========================================================================
# resolve_tenant_id() — Group 2: Fail-closed (INV-1 mandatory filter)
# ===========================================================================


def test_resolve_tenant_id_raises_on_missing_tenant_id() -> None:
...
def test_resolve_tenant_id_raises_on_invalid_uuid_string() -> None:
...
def test_resolve_tenant_id_raises_on_none_tenant_id() -> None:
...
def test_resolve_tenant_id_raises_on_blank_tenant_id() -> None:
...
def test_resolve_tenant_id_non_mapping_session_raises_type_error() -> None:
    """DoD 7: non-mapping session raises TypeError, not PermissionError."""
    with pytest.raises(TypeError, match="dict-like mapping"):
        resolve_tenant_id("not-a-mapping")

    with pytest.raises(TypeError):
        resolve_tenant_id(12345)


# ===========================================================================
# resolve_tenant_id() — Group 3: T1 IDOR prevention
# ===========================================================================


def test_resolve_tenant_id_ignores_client_declared_tenant_in_body() -> None:
    """DoD 5: T1 IDOR prevention.

    The session carries ANKOR_ID server-side. An attacker's fabricated request body
    would carry OTHER_ID — but resolve_tenant_id() never receives the request body.
    ...
    result = resolve_tenant_id(server_session)
